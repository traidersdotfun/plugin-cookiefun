...
import { EnhancedTweet } from "../services/cookiefunApi/types";
...
export const analyzeTweetsAction: Action = {
    ...
    handler: async (
        ...
        try {
            ...
            const cookieService: CookiefunApiService = runtime.getService(CookiefunApiService.serviceType);
            ...
            const formattedResponse = `My analysis:\n\n${analysis}\n\nTop Tweets:\n\n${topTweets.map((tweet, index) => (
                `${index + 1}. @${tweet.authorUsername} (${tweet.formattedDate}) - ${tweet.formattedEngagement}\n${tweet.formattedText}\n`
            )).join('\n')}`;
