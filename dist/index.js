...
// src/services/cookiefunApi/index.ts
import { Service, ServiceType, elizaLogger } from "@elizaos/core";
import axios from "axios";

// src/services/cookiefunApi/config.ts
var COOKIE_CONFIG = {
...
// src/services/cookiefunApi/scoring.ts
function calculateTweetScore(tweet) {
  const baseScore = tweet.likesCount * 1 + tweet.repliesCount * 3 + tweet.retweetsCount * 5 + tweet.quotesCount * 10;
  const engagementBonus = tweet.smartEngagementPoints * 5;
  return baseScore + engagementBonus;
}

// src/services/cookiefunApi/formatters.ts
var formatDate = (dateStr) => {
  const tweetDate = new Date(dateStr);
  const now = /* @__PURE__ */ new Date();
  const diffInSeconds = Math.floor(
    (now.getTime() - tweetDate.getTime()) / 1e3
  );
  if (diffInSeconds < 60) {
    return "just now";
...
var formatEngagement = (tweet) => {
  return `\u{1F44D} ${tweet.likesCount} \u{1F504} ${tweet.retweetsCount} \u{1F4AC} ${tweet.repliesCount}`;
};
function formatCookieData(response) {
  ...
  return response.ok.map((tweet) => ({
    ...
    formattedEngagement: formatEngagement(tweet),
    formattedText: tweet.text.replace(/\n/g, " ")
  }));
}

// src/services/cookiefunApi/index.ts
var CookiefunApiService = class extends Service {
  apiKey;
  baseUrl;
  lastRequestTime = 0;
  runtime;
  ...
  async initialize(runtime) {
    ...
    elizaLogger.info("\u{1F504} Initializing Cookie.fun API Service...");
    try {
  ...
  async checkRateLimit() {
    ...
    if (timeSinceLastRequest < 6e4 / COOKIE_CONFIG.RATE_LIMIT.MAX_REQUESTS_PER_MINUTE) {
      await new Promise((resolve) => setTimeout(
        resolve,
        6e4 / COOKIE_CONFIG.RATE_LIMIT.MAX_REQUESTS_PER_MINUTE - timeSinceLastRequest
      ));
    }
  ...
  async searchTweets(params) {
    ...
    elizaLogger.info("\u{1F50D} Searching tweets for query:", params.query);
    try {
      ...
      elizaLogger.debug("\u{1F4CA} Retrieved tweets:", {
  ...
  async processBatch(queries, maxResults) {
    elizaLogger.debug("\u{1F504} Processing batch of queries:", { count: queries.length });
    const batchPromises = queries.map(
      (query) => this.searchTweets({ query, max_results: maxResults })
    );
    const results = await Promise.all(batchPromises);
    return results.flat();
  }
  async searchMultipleQueries(queries, maxResults = 10) {
    elizaLogger.info("\u{1F50D} Searching multiple queries:", { queries, maxResults });
...
var cookiefunApi_default = CookiefunApiService;

// src/actions/analyzeTweets.ts
var topicExtractionTemplate = `Look at the user's message and identify the main cryptocurrency or token they want to analyze.
...
Last message:
{{text}}`;
var analyzeTemplate = `Analyze these tweets about cryptocurrency and provide a summary of the key insights.
...
var analyzeTweetsAction = {
  ...
  handler: async (runtime, message, state, _options, callback) => {
    ...
    elizaLogger2.info("Starting tweet analysis", { actionId });
    try {
      ...
      const topicFields = results.object;
      ...
      const cookieService = runtime.getService(cookiefunApi_default.serviceType);
      ...
      const formattedTweets = searchResults.map((tweet) => `Tweet: ${tweet.formattedText} (Likes: ${tweet.likesCount}, Retweets: ${tweet.retweetsCount}, Replies: ${tweet.repliesCount}, Quotes: ${tweet.quotesCount}, Smart Engagement: ${tweet.smartEngagementPoints}, Score: ${tweet.score})`).join("\n---\n");
      ...
      const topTweets = searchResults.sort((a, b) => b.score - a.score).slice(0, 3);
      const formattedResponse = `My analysis:

${analysis}

Top Tweets:

${topTweets.map((tweet, index) => `${index + 1}. @${tweet.authorUsername} (${tweet.formattedDate}) - ${tweet.formattedEngagement}
${tweet.formattedText}
`).join("\n")}`;
      if (callback) {
        callback({
      ...
      elizaLogger2.error("Tweet analysis failed:", error);
      if (callback) {
        callback({
...
var analyzeTweets_default = analyzeTweetsAction;

// src/index.ts
var cookiefunPlugin = {
  name: "cookiefun",
  description: "Cookie.fun plugin for Eliza",
  actions: [analyzeTweets_default],
  evaluators: [],
  services: [new cookiefunApi_default()]
};
var index_default = cookiefunPlugin;
export {
  cookiefunPlugin,
